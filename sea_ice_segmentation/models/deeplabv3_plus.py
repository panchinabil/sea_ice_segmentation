...
class ResNet(nn.Module):
    def __init__(
        self, in_channels=3, output_stride=16, backbone="resnet101", pretrained=True
    ):
...
class Block(nn.Module):
    def __init__(
        ...
        rep.append(
            SeparableConv2d(in_channels, out_channels, 3, stride=1, dilation=dilation)
        )
        rep.append(nn.BatchNorm2d(out_channels))

        rep.append(self.relu)
        rep.append(
            SeparableConv2d(out_channels, out_channels, 3, stride=1, dilation=dilation)
        )
        rep.append(nn.BatchNorm2d(out_channels))
        ...
        rep.append(
            SeparableConv2d(
                out_channels, out_channels, 3, stride=stride, dilation=dilation
            )
        )
        rep.append(nn.BatchNorm2d(out_channels))
