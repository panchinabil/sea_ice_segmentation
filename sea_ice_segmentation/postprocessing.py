...
def conv_crf(image, unary, device=None, half_prec=None, argmax=True, config=default_conf):
    if device is None: device = unary.device
    if half_prec is None: half_prec = (image.dtype == toch.float16) or (unary.dtype == torch.float16)

    num_classes = unary.shape[1]
    shape = image.shape[2:]
        
    gausscrf = GaussCRF(conf=config, shape=shape, nclasses=num_classes)
    gausscrf = to_device(gausscrf, device)
    
    image = to_device(image, device)
    unary = to_device(unary, device)
    
    if half_prec:   
        gausscrf = gausscrf.half()
        image = image.half()
        unary = unary.half()
    
    preds = gausscrf(unary=unary, img=image)
    if argmax: preds = preds.argmax(1, keepdim=True)
    return preds



class ProcMetric(LearnerCallback):
    _order = -20 
    def __init__(self, learn, met, postproc_fn, name = None, softmax=False):
        super().__init__(learn)
        self.met, self.name, self.proc_func, self.softmax = met, name, postproc_fn, softmax
        if self.name==None: self.name = self.proc_func.__name__
        

    def on_train_begin(self, **kwargs):
        self.learn.recorder.add_metric_names([f"{self.met.__name__}-{self.name}"])

        
    def on_epoch_begin(self, **kwargs):
        dvc = self.learn.data.device
        self.val = to_device(torch.tensor(0.), self.learn.data.device)
        self.total_n = to_device(torch.tensor(0.), self.learn.data.device)
    
    
    def on_batch_begin(self, last_input, last_target, **kwargs):
        self.last_input = last_input.float()
        return {'last_input': last_input, 'last_target': last_target}  
    
    
    def on_batch_end(self, last_output, last_target, **kwargs):
        if not self.check_training():
            last_output, last_target = last_output.float(), last_target.float()
            if self.softmax: last_output = F.softmax(last_output)
            last_output = self.proc_func(self.last_input, last_output).float()
            last_output = to_device(last_output, self.learn.data.device)
            
            m = self.met(last_output, last_target, argmax=False)
            n = last_output.shape[0]
            self.total_n += n
            self.val += m*n
...
def get_metrics_callbacks(learn, metrics, postproc_fns, **kwargs):
    metrics_callbacks = []
    for m in metrics:
        for f in listify(postproc_fns):
            metrics_callbacks.append(ProcMetric(learn, m, f, **kwargs))
